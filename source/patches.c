...
static int is_sw_line(const char *s)
{
	if (!s) return 0;
	// Pattern: 8 hex + space + 8 hex (Save Wizard style)
...
static char *sw_exec_line(patch_ctx_t *ctx, char *line)
{
	uint8_t *data = *ctx->data;
	size_t dsize = *ctx->size;
	long pointer = *ctx->pointer;
	long end_pointer = *ctx->end_pointer;
	uint32_t ptr_value = *ctx->ptr_value;
...
sw_return_next:
	*ctx->pointer = pointer;
	*ctx->end_pointer = end_pointer;
	*ctx->ptr_value = ptr_value;
	return strtok(NULL, "\n");
...
size_t apply_bsd_patch_code(uint8_t** src_data, size_t dsize, const code_entry_t* code)
{
	...
	patch_ctx_t sw_ctx = { &data, &dsize, &pointer, &end_pointer, &ptr_value };
	for (char *line = strtok(bsd_code, "\n"); line != NULL; )
	{
		strip_cr(line);
		if (is_sw_line(line))
		{
			line = sw_exec_line(&sw_ctx, line);
			continue;
		}
...
size_t apply_sw_patch_code(uint8_t *data, size_t dsize, const code_entry_t* code)
{
	...
	patch_ctx_t ctx = { &data, &dsize, &pointer, &end_pointer, &ptr_value };

	for (char *line = strtok(gg_code, "\n"); line != NULL; )
		line = sw_exec_line(&ctx, line);
